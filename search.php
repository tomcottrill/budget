  ...
  <body>
			...
			<div id="wrapper" class="row-fluid main">
				...
				<div id="search-panel" class="span4 search-sidebar">
					<ul id="search-panel-tabs" class="nav nav-tabs">
						<li class="active"><a href="#tabs-address" data-toggle="tab" data-type="address">Address</a></li>
						<li><a href="#tabs-zip" data-toggle="tab" data-type="zip">Zipcode</a></li>
						<li><a href="#tabs-county" data-toggle="tab" data-type="county">County</a></li>
					</ul>
					<div class="tab-content">
						<div class="tab-pane active" id="tabs-address">
							<form class="form-inline" style="width:100%">
								...
								<button class="btn btn-success" id="btn_address_search">Search</button>
							</form>
						</div>
						<div class="tab-pane" id="tabs-zip">
							<form class="form-inline" style="width:100%">
								...
								<button class="btn btn-success" id="btn_zip_search">Search</button>
							</form>
						</div>
						<div class="tab-pane" id="tabs-county">
							<form class="form-inline" style="width:100%">
								...
								<button class="btn btn-success" id="btn_county_search">Search</button>
							</form>
						</div>							
					</div>
					<div id="results">
				...
				<div id="search-area" class="span8 search-area">
					<ul id="tabs" class="nav nav-tabs">
						<li class="active"><a href="#tabs1-pane1" data-toggle="tab" data-type="detail">Results (MAP)</a></li>
					</ul>
					<div class="tab-content">
		...
		function searchAreaMap(map){
			...
			me.processRows = function(rows){
				...
				for(i=0;i<rows.rows.length;i++){
					...
					me.results.push( 
						{
							'geometry': new GeoJSON( $.parseJSON(record.geom), {strokeColor: "#FF7800",strokeOpacity: 1,strokeWeight: 2,fillColor: "#46461F",fillOpacity: 0.25} ) ,
							'name': record.name,
				...
				if(me.goi != null){
					me.goi = null;					
					me.goi = {'geometry': new GeoJSON( $.parseJSON(rows.goi), {strokeColor: "#000000",strokeOpacity: 1,strokeWeight: 2,fillColor: "#14D400",fillOpacity: 0.25} ), 'type': rows.goi_type};
				}				
				else
				{
					me.goi = {'geometry': new GeoJSON( $.parseJSON(rows.goi), {strokeColor: "#000000",strokeOpacity: 1,strokeWeight: 2,fillColor: "#14D400",fillOpacity: 0.25} ), 'type': rows.goi_type};
	...
	<script>
		$(document).ready(function(){
			...
			$("#search-map").height( ($(window).height() - $("#header").height() - 50 - $("#tabs").height()) / 2);
			...
			$("#btn_zip_search").click(function(e){
				...
				searchController.sendRequest({'type': 'zip', 'zip': $("#search-zip-input").val()}, test);			
			});
			...
			$("#btn_county_search").click(function(e){
				...
				searchController.sendRequest({'type': 'county', 'county': countyValues[0], 'state': countyValues[1].replace(" ", "")}, test);				
			});
			...
			$("#btn_address_search").click(function(e){
				...
				if( $("#search-address-input").val() !="" ){
					...
					searchController.geocoder.geocode({'address': $("#search-address-input").val()}, function(results, status){
						if( status == google.maps.GeocoderStatus.OK){
							var addressPoint = {'type': 'address', 'latitude': results[0].geometry.location.lat(), 'longitude': results[0].geometry.location.lng()};
			...
			function test(data){
								...
								for (var k = 0; k < sgeom.length; k++){
									if(sgeom[k].length){
										for(var j = 0; j < sgeom[k].length; j++){
											...
											sgeom[k][j].setOptions({'zIndex':1000});											
									...
									else
									{
										...
										sgeom[k].setOptions({'zIndex':1000});
							...
							for (var k = 0; k < item.length; k++){
								if(item[k].length){
									for(var j = 0; j < item[k].length; j++){
										...
										$("#company_resultset").append( $('<tr id="provider_' + record['provider_id'] + '" data-provider="'+record['provider_id']+'"><td>'+record['provider_name']+' ('+record['name']+')</td></tr>') );
					...
					$("#company_resultset > tr").click(function(){
						...
						$.post("php/fetch.php",{'id': $(this).attr('data-provider')}, function(data){
