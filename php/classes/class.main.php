		...
		switch($serviceObj->type){
			case "zip":
				...
				for($i=0;$i<count($zipMsg->rows);$i++){
					...
					$sql = "INSERT INTO service_areas (the_geom,name,area_type,provider_id, provider_name, provider_rank) VALUES ".
							  "(ST_GeomFromText('" . $zipMsg->rows[$i]->geom . "',4326),'" . $zipMsg->rows[$i]->zipcode . "','" . $serviceObj->type. "','" . $serviceObj->provider_id . "', '".$serviceObj->provider_name."', '".$serviceObj->rank."')";
			...
			case "county":
				...
				foreach($countyData as $item){
					$sql = "INSERT INTO service_areas (the_geom,name,area_type,provider_id, provider_name, provider_rank) VALUES ".
					"(ST_GeomFromText('" . $item->geom . "',4326),'" .$item->county. ", " .$item->state."','" . $serviceObj->type. "','" . $serviceObj->provider_id . "', '".$serviceObj->provider_name."', '".$serviceObj->rank."')";
			...
			case "radius":
				...
				foreach($serviceObj->radii as $key=>$value){
					...
					if( $key < (count($serviceObj->radii)-1) ){
						...
						$sql = "INSERT INTO service_areas (the_geom,area_type,name,provider_id,provider_name, provider_rank) VALUES (".
										   "ST_GeomFromText(".
												"(SELECT ST_Astext(ST_Difference((SELECT ST_Multi(ST_Transform(".
													"ST_Transform(".
														"ST_Buffer(".
															"ST_Transform(".
																"ST_GeomFromText('POINT(".$serviceObj->centerLng." ".$serviceObj->centerLat .")',4326),".
																	"_ST_BestSRID(".
																		"ST_GeomFromText('POINT(".$serviceObj->centerLng." ".$serviceObj->centerLat .")' ,4326))),".$milesA."),3857),4326))) ,".
																				...
																				"ST_Transform(".
																					"ST_Buffer(".
																						"ST_Transform(".
																							"ST_GeomFromText('POINT(".$serviceObj->centerLng." ".$serviceObj->centerLat .")',4326),".
																								"_ST_BestSRID(".
																									"ST_GeomFromText('POINT(".$serviceObj->centerLng." ".$serviceObj->centerLat .")' ,4326))),".$milesB."),3857),4326))))  ) ) ".
																										"),4326), '".$serviceObj->type."', 'radius area(".$serviceObj->radii[$key]."-".$serviceObj->radii[intval($key+1)]." mi)', '".$serviceObj->provider_id."','".$serviceObj->provider_name."'".
																											",'".$serviceObj->rank."')";
			...
			case "custom":
				...
				$sql = "INSERT INTO service_areas (the_geom,name,area_type,provider_id,provider_name, provider_rank) VALUES ".
					"(ST_GeomFromText('" . $geometry . "',4326),'Custom defined service area','" . $serviceObj->type . "','" . $serviceObj->provider_id . "','".$serviceObj->provider_name."','".$serviceObj->rank."')";
...
class dumpsterManager{
	...
	public function getDetails($providerID){
		$query = "SELECT * FROM " . TBL_PROVIDERS . " WHERE provider_id='" . $providerID . "'";
		$result = $this->dbc->performQuery($query);
